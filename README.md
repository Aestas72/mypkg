# mypkg — コマンド実行ログROS２パッケージ
![test](https://github.com/Aestas72/mypkg/actions/workflows/test.yml/badge.svg)

`mypkg`は、ROS２上でシェルコマンドを定期的に実行し、その実行結果をトピック通信で配信・表示するパッケージです。

システム状態確認(ディスク使用量やプロセス状態など)をROS２ノードとして扱えるため，監視ノードやログ収集ノードと組み合わせて利用できます。
